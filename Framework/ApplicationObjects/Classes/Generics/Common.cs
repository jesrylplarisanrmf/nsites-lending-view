...
namespace NSites_V.ApplicationObjects.Classes.Generics
{
    class Common
    {
        ...
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddressCORE);
        ...
        public DataTable getCurrentFinancialYear()
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddressCORE);
            HttpResponseMessage response = client.GetAsync("api/main/getCurrentFinancialYear").Result;
            return response.Content.ReadAsAsync<DataTable>().Result;
        }
            ...
            string pSupplierId, string pCustomerId, string pUserId, string pRemarks)
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddressCORE);
            HttpResponseMessage responseAdd = client.GetAsync("api/main/insertJournalEntryFromOutside?pFinancialYear=" + pFinancialYear + "&pJournal=" +
                pJournal + "&pForm=" + pForm + "&pVoucherNo=" + pVoucherNo + "&pDatePrepared=" + pDatePrepared + "&pExplanation=" + pExplanation +
                "&pTotalDebit=" + pTotalDebit + "&pTotalCredit=" + pTotalCredit + "&pReference=" + pReference + "&pSupplierId=" + pSupplierId +
                "&pCustomerId=" + pCustomerId + "&pUserId=" + pUserId + "&pRemarks=" + pRemarks + "").Result;
            return responseAdd.Content.ReadAsStringAsync().Result;
            /*
        ...
        public string insertJournalEntryDetailFromOutside(string pJournalEntryId, string pAccountId, decimal pDebit, decimal pCredit,
            string pSubsidiary, string pSubsidiaryId, string pSubsidiaryDescription, string pRemarks, string pUserId)
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(GlobalVariables.BaseAddressCORE);
            HttpResponseMessage responseAdd = client.GetAsync("api/main/insertJournalEntryDetailFromOutside?pJournalEntryId=" + pJournalEntryId +
                "&pAccountId=" + pAccountId + "&pDebit=" + pDebit + "&pCredit=" + pCredit + "&pSubsidiary=" + pSubsidiary + "&pSubsidiaryId=" + pSubsidiaryId +
                "&pSubsidiaryDescription=" + pSubsidiaryDescription + "&pRemarks=" + pRemarks + "&pUserId=" + pUserId + "").Result;
            return responseAdd.Content.ReadAsStringAsync().Result;
        }
